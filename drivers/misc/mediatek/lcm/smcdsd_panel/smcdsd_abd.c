...
{
	unsigned int mask = 0, print_tag, bit;
	...
	for_each_set_bit(bit, (unsigned long *)&print_tag, bit_size) {
		if (print[bit])
...
static void smcdsd_abd_print_str(struct seq_file *m, struct abd_str *trace)
{
	...
	for (i = 0; i < log_max; i++) {
		...
		seq_printf(&p, "%d-%02d-%02d %02d:%02d:%02d / %lu.%06lu / %-20s ",
			tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec,
			(unsigned long)ts.tv_sec, ts.tv_nsec / 1000, log->print);
	}
